...
# === ДИНАМИЧЕСКИЙ ГОРИЗОНТ В СЕКУНДАХ ===
INTERVALS = [
...
def update_weights_smart(ww, pred_up, prediction_price, current_price, factor,
                        atr=None, min_move_threshold=0.12, forgetting_factor=0.985):
    """
    Умная корректировка весов с учётом силы движения + затухание.
    """
    if not isinstance(ww, dict):
        ww = {'up_weight': 1.0, 'down_weight': 1.0, 'correct': 0, 'total': 0, 'streak': 0}

    price_change_pct = (current_price - prediction_price) / prediction_price * 100 if prediction_price != 0 else 0
    actual_up = price_change_pct > 0
    move_strength = abs(price_change_pct)

    if atr and atr > 0:
        noise_threshold = (atr / prediction_price * 100) * 0.75
    else:
        noise_threshold = min_move_threshold

    is_strong_move = move_strength >= noise_threshold * 1.8
    is_small_noise = move_strength < noise_threshold * 0.7
    ...
    if prediction_correct:
        if is_strong_move:
            ww[key] *= (1 + factor * 1.35)
            ww['streak'] = ww.get('streak', 0) + 2
        elif is_small_noise:
            ww[key] *= (1 + factor * 0.3)
            ww['streak'] = ww.get('streak', 0) + 1
        else:
            ww[key] *= (1 + factor * 0.85)
            ww['streak'] = ww.get('streak', 0) + 1
        ww['correct'] = ww.get('correct', 0) + 1
    else:
        if is_small_noise:
            ww[key] *= (1 - factor * 0.45)
        elif is_strong_move:
            ww[key] *= (1 - factor * 2.1)
            ww['streak'] = 0
        else:
    ...
    # Затухание весов
    ww['up_weight'] = 1.0 + (ww.get('up_weight', 1.0) - 1.0) * forgetting_factor
    ww['down_weight'] = 1.0 + (ww.get('down_weight', 1.0) - 1.0) * forgetting_factor

    ww['up_weight'] = max(0.18, min(5.0, ww['up_weight']))
    ww['down_weight'] = max(0.18, min(5.0, ww['down_weight']))

    ww['total'] = ww.get('total', 0) + 1
    ww['accuracy'] = ww['correct'] / ww['total'] if ww['total'] > 0 else 0.5
...
def get_futures_pairs():
    try:
        ...
        pairs = [s['symbol'] for s in info['symbols'] if s.get('contractType') == 'PERPETUAL']
        tickers = client.futures_ticker()
        volume_dict = {t['symbol']: float(t['quoteVolume']) for t in tickers}
        active_pairs = [p for p in pairs if volume_dict.get(p, 0) > 1]
        logging.info(f"Получено {len(active_pairs)} активных пар")
        return active_pairs
...
def get_historical_data(symbol, interval, limit=None):
    ...
    try:
        ...
        # Добавляем колонки для объёма лонг/шорт
        ...
        logging.info(f"Получено {len(df)} строк для {symbol} (limit={limit})")
        with cache_lock:
...
def monitor_tremor(symbol, interval_seconds, n_samples=20, max_retries=3):
    ...
    try:
        ...
        wst.start()
        time.sleep(0.8)  # Даем подключиться

        # Ждём данные или таймаут
        max_wait = 12  # ←←←← КЛЮЧЕВОЕ ИЗМЕНЕНИЕ
        while len(prices) < n_samples and time.time() - start_time < max_wait:
            time.sleep(0.15)
...
class CryptoApp:
    def __init__(self, root):
        ...
        self.root.geometry("1750x720")  # Ширина для новых колонок
    ...
    def setup_ui(self):
        ...
        cols = ('Pair', 'Up', 'Down', 'Diff', 'Change', 'Decision', 'Acc', 'N-Candles', 'Tremor', 'Streak', 'Long Vol',
                'Short Vol', 'Pred Time', 'Pred Price', 'Verif Time', 'Verif Price', 'Fund(bps)', 'Fund MA', 'Curr Fund', 'L/S Ratio', 'OI')
        widths = [70, 45, 45, 45, 55, 70, 45, 60, 60, 45, 70, 70, 100, 70, 100, 70, 60, 60, 70, 50, 70]
    ...
    def display_results(self, results):
        ...
        for r in results:
            if r:
                ...
                verif_price = round(r.get('verification_price', 0), 6) if r.get('verification_price') != '-' else '-'
                ...
                self.tree.insert('', 'end', values=(
                    ...
                    r['predicted_change'], r['decision'], f"{acc}%",
                    n_dir, tremor_str, streak, long_vol, short_vol, pred_time, pred_price, verif_time, verif_price,
                    funding_rate, funding_ma, curr_funding, long_short_ratio, oi
                ))
                save_json(RECOMMENDED_TRADE_FILE, r)  # Теперь с timestamp
    ...
    def single_training_cycle(self, symbols, horizon, wait, alpha, multiplier):
        ...
        with ThreadPoolExecutor(max_workers=5) as exec:
            futures = [exec.submit(predict_symbol, s, horizon, lstm, w, self.history_candles, use_n, n_val, use_dev) for
                       s in symbols]
        ...
        self.root.after(0, lambda: self.update_training_status("Проверка...", "blue"))
            ...
            for p in predictions:
                ...
                try:
                    ...
                    if sym not in weights:
                        weights[sym] = {'up_weight': 1.0, 'down_weight': 1.0, 'correct': 0, 'total': 0, 'streak': 0}
                    ...
                    logging.info(
                        f"[{sym}] {'ВЕРНО ✅' if (pred_up == (current_price > p['prediction_price'])) else 'ОШИБКА ❌'} | "
                        f"Изменение: {price_change_pct:+.2f}% | Точность: {ww['accuracy']:.1%} | "
                        f"Streak: {ww.get('streak', 0)} | UpW: {ww.get('up_weight', 1):.2f} | DownW: {ww.get('down_weight', 1):.2f}"
                    )

                except Exception as e:
                    logging.error(f"Ошибка обновления весов для {sym}: {e}")
